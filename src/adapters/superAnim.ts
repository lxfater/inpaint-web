...
// Multi thread max - 2x.StarSample.V1.0.-.FP32.OPSET17 - https://openmodeldb.info/models/2x-MLP-StarSample-V1-0
const multi = 4
// Upscale format
const scal = 2
...
/****/
  ...
  const tileSize = 256 //  512 // 256 // 256 low speed // or 128 good // or 64 very slow
  const tilePadding = 32 // 64
...
export default async function superAnim(
  ...
  if (!model) {
    ...
    const modelBuffer = await ensureModel('superanim')
